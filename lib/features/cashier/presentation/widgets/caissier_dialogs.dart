...
static Future<Map<String, dynamic>?> showRewardsCelebration(
  ...
  try {
    ...
    final result = await showDialog<Map<String, dynamic>>(
      ...
      builder: (dialogContext) => Dialog(
        ...
        child: RewardsCelebrationSheet(
          ...
          onExchangeRewards: () {
            // Ce callback sera appelé APRÈS la fermeture du dialogue
            // On ne fait rien ici, juste pour la compatibilité
          },
          onSaveLater: () {
            // Pareil ici
          },
    ...
    // ✅ Si l'utilisateur a choisi d'échanger, retourner les données complètes
    if (result != null && result['action'] == 'exchange') {
      return {
        'action': 'exchange',
        'clientId': clientId,
        'magasinId': magasinId,
    ...
    // En cas d'erreur, afficher quand même le dialogue sans récompenses
    if (!context.mounted) return null;
